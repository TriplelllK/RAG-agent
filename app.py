...
LMSTUDIO_MODEL = "Meta-Llama-3-8B-Instruct.Q4_K_M.gguf"  # имя из LM Studio

st.set_page_config(page_title="RAG — У-300 КТЛ-1", layout="wide")
st.title("RAG по документации У-300 КТЛ-1")

with st.expander("Настройки поиска"):
    top_k = st.slider("k (выдача из векторного поиска)", 1, 10, 6)
    top_rerank = st.slider("top_k после rerank", 1, 10, 4)
...
if 'store' not in st.session_state:
    ...
    if not (os.path.exists(idx) and os.path.exists(meta)):
        st.error("Не найден индекс. Сначала запустите ingest.py")
        st.stop()
...
if st.button("Ответить") and q.strip():
    ...
    result = make_answer_llm(q, ctx, model=LMSTUDIO_MODEL)
    ...
    with st.expander("Контекст (фрагменты)"):
        for ch in ctx:
            ...
            st.text(ch.text)
with st.expander("Дебаг — структурные данные"):
